...
impl PopupManager {
    ...
    pub fn find_popup(&self, surface: &WlSurface) -> Option<PopupKind> {
        self.unmapped_popups.iter()
        .find(|p| p.get_surface() == Some(surface))
        .cloned()
        .or_else(|| self.popup_trees
            .iter()
    ...
    pub fn popups_for_surface(surface: &WlSurface) -> Result<impl Iterator<Item = (PopupKind, Point<i32, Logical>)>, DeadResource> {
...
impl PopupNode {
    ...
    fn iter_popups_relative_to<P: Into<Point<i32, Logical>>>(&self, loc: P) -> impl Iterator<Item = (&PopupKind, Point<i32, Logical>)> {
        let relative_to = loc.into() + self.surface.location();
        std::iter::once((&self.surface, relative_to)).chain(
            self.children
                .iter()
                .map(move |x| Box::new(x.iter_popups_relative_to(relative_to)) as Box<dyn Iterator<Item = (&PopupKind, Point<i32, Logical>)>>)
                .flatten(),
        )
    }
