...
use smithay::{
    ...
    desktop::{Kind as SurfaceKind, PopupKind, PopupManager, Space, Window},
    reexports::{
...
impl PointerGrab for MoveSurfaceGrab {
    fn motion(
        ...
        self.space
            .borrow_mut()
            .map_window(&self.window, new_location.to_i32_round());
    }
        ...
        move |shell_event, mut ddata| {
            ...
            match shell_event {
                XdgRequest::NewToplevel { surface } => {
                    ...
                    let output = state.space.borrow().outputs().next().cloned();
                    let output_geometry = output
                        .and_then(|o| state.space.borrow().output_geometry(&o))
                    ...
                    state.space.borrow_mut().map_window(&window, (x, y).into());
                }
                ...
                XdgRequest::NewPopup { surface, positioner } => {
                    ...
                    */
                    ...
                    debug!(log_ref, "New popup");
                    state.popups.borrow_mut().track_popup(PopupKind::from(surface));
                }
                ...
                XdgRequest::UnFullscreen { surface } => {
                    ...
                    if let Ok(output) = ret {
                        ...
                        if let Some(output) = output {
                            if let Some(fullscreen) = Output::from_resource(&output)
                                .unwrap()
                                .user_data()
                                .get::<FullscreenSurface>()
                            {
...
fn surface_commit(surface: &wl_surface::WlSurface, space: &RefCell<Space>, popups: &mut PopupManager) {
    ...
    if let Some(window) = space.window_for_surface(surface).cloned() {
        ...
        if let Some(location) = new_location {
            slog_scope::debug!("MOVING WINDOW: {:?}", location);
